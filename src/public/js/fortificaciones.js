document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchCatalog() {
        try {
            ...
            const fortifications = rawItems.filter(item => 
                item.classification === 'fortificaciones' && item.parentId === null
            );

            renderCatalog(fortifications);
        } catch (error) {
            console.error(error);
            catalogContainer.innerHTML = `
                <div class="col-span-full text-center text-red-500 py-12 font-mono">
                    ⚠️ Error al cargar las fortificaciones tácticas: ${error.message}
                </div>`;
        }
    }
    ...
    function renderCatalog(items) {
        if (items.length === 0) {
            catalogContainer.innerHTML = `<div class="col-span-full text-center text-muted-foreground py-12">No hay fortificaciones registradas.</div>`;
            return;
        }

        catalogContainer.innerHTML = items.map(item => {
            const coverImage = item.file ? `/${item.file.url}` : '/images/fortificaciones.png';
    ...
    function updateStep() {
        ...
        if (total === 0) {
            modalImage.src = currentCatalogItem.file ? `/${currentCatalogItem.file.url}` : '/images/fortificaciones.png';
            modalImage.alt = currentCatalogItem.name;
            stepTitle.textContent = "Sin pasos registrados";
            stepContent.textContent = "Este sistema estructural no cuenta con instrucciones asíncronas de despliegue.";
        ...
        modalImage.src = activeStep.file ? `/${activeStep.file.url}` : (currentCatalogItem.file ? `/${currentCatalogItem.file.url}` : '/images/fortificaciones.png');
