...
router.post('/catalog-stepper', protect, restrictToAdmin, uploadStepperFiles, prepareStepperData, validateBody(catalogStepperSchema), async (req, res, next) => {
    ...
    const processCatalog = async () => {
        ...
        if (!files || !files['coverImage']) throw new AppError('La imagen de portada es obligatoria', 400);
        ...
        for (let i = 0; i < parsedSteps.length; i++) {
            ...
            if (!stepImage) throw new AppError(`Falta la imagen para el paso número ${i + 1}`, 400);
...
router.put('/catalog/:id', protect, restrictToAdmin, uploadStepperFiles, validateBody(catalogStepperSchema), async (req, res, next) => {
    const { id } = req.params;
    const t = await sequelize.transaction();
